...
alter table public.transactions
  ...
  check (source in ('manual', 'income_rule', 'purchase', 'expense_rule', 'transfer_rule'));
...
create table public.transfer_rules (
  ...
  constraint transfer_rules_accounts_chk check (from_account_id <> to_account_id),
  constraint transfer_rules_frequency_params_chk check (
...
create index transfer_rules_from_account_id_idx on public.transfer_rules (from_account_id);
create index transfer_rules_to_account_id_idx on public.transfer_rules (to_account_id);

create table public.transfer_occurrences (
  id uuid primary key default gen_random_uuid(),
  transfer_rule_id uuid not null references public.transfer_rules (id) on delete cascade,
  occurred_on date not null,
  status text not null check (status in ('posted', 'skipped', 'adjusted')),
  transaction_id uuid references public.transactions (id) on delete set null,
  created_at timestamptz not null default now(),
  unique (transfer_rule_id, occurred_on)
);

create trigger transfer_rules_set_updated_at
before update on public.transfer_rules
for each row execute function public.set_updated_at();

alter table public.transfer_rules enable row level security;
alter table public.transfer_occurrences enable row level security;

create policy transfer_rules_select on public.transfer_rules
...
create policy transfer_rules_insert on public.transfer_rules
...
create policy transfer_rules_update on public.transfer_rules
...
create policy transfer_rules_delete on public.transfer_rules
...
create policy transfer_occurrences_select on public.transfer_occurrences
for select to authenticated
using (
  exists (
    select 1 from public.transfer_rules tr
    where tr.id = transfer_rule_id
      and public.is_account_member(tr.from_account_id)
      and public.is_account_member(tr.to_account_id)
  )
);

alter table public.transfer_rules replica identity full;
alter table public.transfer_occurrences replica identity full;

do $$
begin
  alter publication supabase_realtime add table public.transfer_rules;
exception when duplicate_object then null;
end $$;
do $$
begin
  alter publication supabase_realtime add table public.transfer_occurrences;
exception when duplicate_object then null;
end $$;

create or replace function public.apply_due_transfer_rules(p_as_of date default current_date)
  ...
  loop
    ...
    loop
      ...
      values (
        ...
        coalesce(nullif(trim(v_rule.title), ''), 'Перевод'),
      ...
      where id = v_rule.from_account_id;
...
revoke all on function public.apply_due_transfer_rules(date) from public, anon;
grant execute on function public.apply_due_transfer_rules(date) to authenticated;

create or replace function public.skip_transfer_occurrence(p_occurrence_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  select * into v_occ from public.transfer_occurrences where id = p_occurrence_id for update;
  ...
  if v_occ.transaction_id is not null then
    ...
    if not exists (
      select 1 from public.transfer_rules tr
      where tr.id = v_occ.transfer_rule_id
        and public.is_account_member(tr.from_account_id)
        and public.is_account_member(tr.to_account_id)
    ) then
      raise exception 'Not an account member';
...
revoke all on function public.skip_transfer_occurrence(uuid) from public, anon;
grant execute on function public.skip_transfer_occurrence(uuid) to authenticated;

create or replace function public.adjust_transfer_occurrence(p_occurrence_id uuid, p_new_amount numeric)
...
begin
  ...
  perform public.apply_posted_balance(
    v_tx.kind, v_tx.account_id, v_tx.counterparty_account_id, v_delta, v_user_id, 1
  );
...
revoke all on function public.adjust_transfer_occurrence(uuid, numeric) from public, anon;
grant execute on function public.adjust_transfer_occurrence(uuid, numeric) to authenticated;
...
begin
  ...
  if v_tx.source = 'transfer_rule' then
    v_account_id := v_tx.account_id;
    v_counterparty_id := v_tx.counterparty_account_id;
    v_occurred_on := v_tx.occurred_on;
  end if;
